...
import {
  PromotionProductDTO
}                                      from "../application/promotion_product_dto"

export const combineProducts = async ( searchProducts: SearchProduct,
  dtos: PromotionProductDTO[] ): Promise<Either<BaseException[], PromotionProduct[]>> => {
  ...
  for ( const product of products ) {
    ...
    if ( quantity ) {
      const pp = PromotionProduct.fromPrimitives( product, quantity )
